...
```
As displayed above, one can have multiple pwa-studio project in the same repository as long as the project directory starts with `pwa-studio`
 ...
 jobs:
   magento2-build:
     ...
     - name: 'this step will build an magento artifact'
       if: (!cancelled())
       uses: MAD-I-T/magento-actions@v3.34
       env:
...
If some issues are encountered on 2.3.X version, please use the **v2.0** of the action in place of **v3.34** 
